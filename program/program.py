...
API_KEY = "308808a4-2ebf-4f40-94cc-0b4d8d90327f" # Your API KEY here
API_URL = "http://api.airvisual.com/v2/city" 
EXCEL_FILE_PATH = "air_quality_data.xlsx"

def fetch_data_from_api():
    params = {
        "city": "Buenos Aires", # Your city
        "state": "Buenos Aires", # Your state
        "country": "Argentina", # Your country
        "key": API_KEY
    }
...
while pd.to_datetime("now") < end_datetime:
    ...
    if data:
        ...
        data_df.to_csv('AQI_PM2_5_Buenos_Aires.csv', mode='a', index=False, header=not pd.DataFrame(columns=columns).empty)
